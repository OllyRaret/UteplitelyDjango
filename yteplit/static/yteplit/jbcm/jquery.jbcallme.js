...
(function ($, window, document, undefined) {
    ...
    Plugin.prototype.build = function () {
        ...
        if (!$("#jbCallme_" + this.options.postfix).length) {
            $('<div id="jbCallme_' + this.options.postfix + '" class="jbCallme"><div class="mnbv modal-content"><div class="jb_title">' + this.options.title + '</div><a title="\u0417\u0430\u043a\u0440\u044b\u0442\u044c" class="jb_close">\u0417\u0430\u043a\u0440\u044b\u0442\u044c</a><form class="jb_form"></form><div class="jb_success">' + this.options.success + '</div><div class="jb_progress"></div><div class="jb_fail">' + this.options.fail + '</div></div></div>').appendTo($("body"));
            ...
            $.each(this.options.fields, function (index, value) {
                ...
                } else if (value.type && value.type == 'submit') {
                    form_input = '<input class="btn btn-black" ' +
                        (value.required ? 'required="required" ' : '') +
                        (value.placeholder ? 'placeholder="' + value.placeholder + '" ' : '') +
                        (value.class ? 'class="' + value.class + '" ' : '') +
                        (value.value ? 'value="' + value.value + '" ' : '') +
                        'type="' + (value.type && jQuery.inArray(value.type, ["submit"]) >= 0 ? value.type : 'text') + '" name="' + index + '" />';
                } else {
                    form_input = '<input class="form-control" ' +
                        (value.required ? 'required="required" ' : '') +
                        (value.placeholder ? 'placeholder="' + value.placeholder + '" ' : '') +
                        (value.class ? 'class="' + value.class + '" ' : '') +
                        (value.value ? 'value="' + value.value + '" ' : '') +
                        'type="' + (value.type && jQuery.inArray(value.type, ["hidden", "text", "email", "date", "color"]) >= 0 ? value.type : 'text') + '" name="' + index + '" />';
                }

                $((value.type != 'hidden' ? '<div class="jb_input form-group">' +
    ...
    Plugin.prototype.end = function () {
        ...
        $('.jbCallme').css({'padding-left': window.innerWidth - document.documentElement.clientWidth + 'px'});
        $('#navigation').css({'padding-right': '0px'});
    ...
    Plugin.prototype.show = function () {
        ...
        $('.jbCallme').css({'padding-left': '0px'});
        $('#navigation').css({'padding-right': window.innerWidth - document.documentElement.clientWidth + 'px'});
